class Type_order {
    static async getBySysname({sysname}, query) {
        return new Promise((prom_result, prom_error) => {
            query(`SELECT * FROM Type_order WHERE sysname = '${sysname}'`, function(query_err, query_rows, fields) {
    ...
    static async getById({id}, query) {
        return new Promise((prom_result, prom_error) => {
            query(`SELECT * FROM Type_order WHERE id = '${id}'`, function(query_err, query_rows, fields) {
                if (query_err) return prom_error(query_err);
